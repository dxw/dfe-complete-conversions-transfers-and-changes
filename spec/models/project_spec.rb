  ...
  describe "Validations" do
    ...
    describe "#urn" do
      ...
      describe "enforcing the uniqueness of the URN (db trigger EnforceUniquenessOfProjectUrn)" do
        ...
        context "when an _active_ or _inactive_ project exists with a given URN" do
          before do
          ...
          [:active, :inactive].each do |state|
            it "is NOT possible to create another _#{state}_ project with that URN" do
              aggregate_failures do
                expect {
                  FactoryBot.create(:transfer_project, urn: 222222, state: state)
                }.to raise_error(ActiveRecord::StatementInvalid, /TinyTds::Error/)

                expect {
                  FactoryBot.create(:transfer_project, urn: 333333, state: state)
                }.to raise_error(ActiveRecord::StatementInvalid, /TinyTds::Error/)
              end
            end
          end
